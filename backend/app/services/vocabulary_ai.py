"""AI-powered additions to the Vocabulary feature, all backed by the same
local LLM Conversation uses (see app/services/voice/llm_chat_engine.py) —
no separate model, no cloud calls, nothing fabricated when the model can't
answer. Gated the same way Conversation gates real AI use: the model must
be both downloaded (_ensure_ready) and actually loaded into memory via the
explicit "Launch AI" action (_ensure_launched), or these raise
EngineUnavailable with the same actionable message Conversation uses.
"""
...
def _llm_option(conn: sqlite3.Connection, user_id: str) -> conversation.model_catalog.LlmOption:
    conversation._ensure_ready(conn, user_id, "text")
    option = conversation.selected_llm_option(conn, user_id)
    conversation._ensure_launched({"llm"}, llm_option=option)
    return option
...
def explain_in_context(conn: sqlite3.Connection, *, user_id: str, word: str, context: str) -> dict:
    ...
    option = _llm_option(conn, user_id)
    system_prompt = (
    ...
    data = llm_chat_engine.generate_json(
        system_prompt, user_prompt, repo_id=option.repo_id, filename=option.filename, max_tokens=250
    )
    return {
        "word": word.strip(),
...
def generate_examples(conn: sqlite3.Connection, *, user_id: str, word_row: sqlite3.Row, count: int = 3) -> list[str]:
    ...
    data = llm_chat_engine.generate_json(
        system_prompt, user_prompt, repo_id=option.repo_id, filename=option.filename, max_tokens=300
    )
...
def generate_mnemonic(conn: sqlite3.Connection, *, user_id: str, word_row: sqlite3.Row) -> str:
    ...
    user_prompt = f"Word: {word_row['word']}\nDefinition: {definition}"
    data = llm_chat_engine.generate_json(
        system_prompt, user_prompt, repo_id=option.repo_id, filename=option.filename, max_tokens=120
    )
